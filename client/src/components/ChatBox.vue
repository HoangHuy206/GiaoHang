  ...
  <div class="flex flex-col h-[450px] w-full bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden font-sans">
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 scroll-smooth" ref="messageContainer">
      ...
      <div v-for="msg in messages" :key="msg.id || msg.tempId" 
           :class="['flex w-full group', String(msg.sender_id) === String(currentUser.id) ? 'justify-end' : 'justify-start']">
        ...
        <div :class="[
          ...
          <!-- Debug Log (Ẩn đi sau khi fix xong) -->
          <!-- <span class="text-[8px] text-red-500 hidden">ID: {{msg.sender_id}} ({{typeof msg.sender_id}}) vs {{currentUser.id}} ({{typeof currentUser.id}})</span> -->
          <p class="text-[15px] leading-relaxed break-words">{{ msg.content }}</p>
          <div :class="['text-[10px] mt-1 flex items-center gap-1', String(msg.sender_id) === String(currentUser.id) ? 'text-green-100 justify-end' : 'text-gray-400']">
